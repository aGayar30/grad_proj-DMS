        ...
        for (const auto& out : outs) {
            for (int i = 0; i < out.rows; ++i) {
                ...
                if (confidence > confThreshold && !firstFaceCropped) {
                    ...
                    cv::equalizeHist(grayFace, equalizedFace);

                    // Resize for consistency
                    cv::Mat resizedFace;
                    cv::resize(equalizedFace, resizedFace, cv::Size(100, 100)); // Adjust the size as needed

                    // Scale the pixel values if necessary (assuming already done by blobFromImage)
