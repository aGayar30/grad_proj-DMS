...
int main() {
    ...
    DMSManager dmsManager(cameraQueue, preprocessingQueue, faceDetectionQueue, drowsinessDetectionQueue, tcpOutputQueue, tcpPort, stateOutputQueue, postOutputQueue,commandsQueue,faultsQueue);

    // Initialize the camera component
    if (!dmsManager.initializeCamera("/home/dms/Downloads/drowsiness_detection-master/src/sample_videos/sleepy.mp4")) { // use /dev/video0 for camera or /path/to/video/file
    ...
    cv::Mat cameraFrame, preprocessedFrame, faceDetectedFrame, drowsyDetectedFrame;
    while (true) {
